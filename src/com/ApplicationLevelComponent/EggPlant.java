package com.ApplicationLevelComponent;
...
public class EggPlant
{
	...
	public static String getValueFromExcel(String Key)
	{
	  ...
	  public void startEggplantProcess() { try {
	  TestCaseRunner.startEggDriveProcess();
	 HtmlResult.passed("To start Eggplant process",
	  "Eggplant process should be start successfully",
	  "Eggplant process starts successfully"); } catch (Exception e) {
	  HtmlResult.failed("To start Eggplant process",
	  "Eggplant process should be start successfully",
	 "Starting Eggplant process failed"); } }
	 
	  public void stopEggplantProcess() { try {
	 TestCaseRunner.stopEggDriverProcess();
	  HtmlResult.passed("To stop Eggplant process",
	 "Eggplant process should be stop successfully",
	  "Eggplant process stop successfully"); } catch (Exception e) {
	  HtmlResult.failed("To stop Eggplant process",
	  "Eggplant process should be stop successfully",
	 "Stopping Eggplant process failed"); } }
...

	{
		...
		try {
			...
			HtmlResult.failed("ScreenPart Method", "Screen Part selection should be passed ",
					"Screen part selection is failed");
			e.printStackTrace();
		}
		return null;
	...
	public void pressHomeButton() {
		try {
			...
			HtmlResult.passed("To press home button", "Home button should be pressed successfully",
					"Home button pressed successfully");
		} catch (Exception e) {
			HtmlResult.failed("To press home button", "Home button should be pressed successfully",
					"Home button not pressed successfully-" + e);
		}
	}
	...
	public void typeText(Map Text) {
		...
		try {
			...
			HtmlResult.passed("To type the text ", "Text-'" + Text + "' should be typed sucessfully",
					"Text typed successfully:'" + Text + "'");
		} catch (Exception e) {
			HtmlResult.failed("To type the text ", "Text-'" + Text + "' should be typed sucessfully",
					"cannot type text:'" + Text + "'- " + e);
		}
	}
		...
		try {
			...
			for (String ButtonName : strArrButtonNames) {
				...
				if (ImageFound) {
					...
					if (ImageClicked) {
						HtmlResult.passed(strStepDesc, strExpected,
								"Button-'" + ButtonName + "' found and clicked successfully");
						Result = true;
					} else {
			...
			// HtmlResult.failed(strStepDesc,strExpected,"Button-'"+ ButtonNames +"' not
			// found '" + e.getMessage() + "'");
			return false;
		}
	...
	public boolean completePreviousOrder() {
		try {
			...
			if (SalesPanelOrders.equals("") || SalesPanelOrders.contains("END OF SALE")
					|| SalesPanelOrders.length() <= 1) {
				return true;
			} else {
